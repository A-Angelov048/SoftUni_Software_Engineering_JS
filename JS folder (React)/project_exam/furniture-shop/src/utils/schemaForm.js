...
export const reviewSchema = yup.object().shape({
  rating: yup
    .string()
    .required("From 1 to 5 - how much do you like this furniture?"),
  review: yup
    .string()
    .min(8, "Review must be at least 8 characters")
    .required(
      "What's the point of an empty review? Help other customers make the right choice."
    ),
});
...
export const deliveryFormSchema = yup.object().shape({
  ...
  "zip-code": yup
    .string()
    .matches(/^\d{4}$/, "Invalid zip code (example: 1000)")
  ...
  phone: yup
    ...
    .matches(
      /^\+359[- ]?\d{3}[- ]?\d{3}[- ]?\d{3}$/,
      "Invalid phone number (example: +359111222333)"
    )
    .required("Required field"),
});
