    ...
    return (
        <section className="brand-section layout-padding">
            ...
            <div className='pagination'>
                {statePage > 1 && <i onClick={() => handlePageChange('decrement')} className='bx bx-first-page' ></i>}
                {lengthPages.length > 1 && lengthPages.map((page, index) => (
                    <p onClick={() => handlePageChange(page)} key={index} className={statePage === page ? 'page-number active' : 'page-number'}>{page}</p>
                ))}
                {statePage < lengthPages.length && <i onClick={() => handlePageChange('increment')} className='bx bx-last-page'></i>}
            </div>
